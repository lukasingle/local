...
namespace Example8_9_1
{
    ...
    public class DrawChart : ISerializable
    {
        ...
        private SliceEnum _mxyzSlice = SliceEnum.XSlice;
        private float _msliceLocation = 0;
        #endregion
        ...
        {
            get { return _msliceLocation; }
            set { _msliceLocation = value; }
        }
        ...
        {
            get { return _mxyzSlice; }
            set { _mxyzSlice = value; }
        }
        #endregion
        ...
        protected DrawChart(SerializationInfo info, StreamingContext context)
        {
            ...
            _mxyzSlice = (SliceEnum)info.GetValue("xyzSlice", typeof(SliceEnum));
            _msliceLocation = info.GetSingle("sliceLocation");
        }
        /// <summary>
        ...
        public virtual void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            info.AddValue("xyzSlice", _mxyzSlice);
            info.AddValue("sliceLocation", _msliceLocation);
        ...
        private void AddContour(Graphics g, DataSeries ds, ChartStyle cs, ChartStyle2D cs2d)
        {
            ...
            float zmin = ds.ZDataMin();
            float zmax = ds.ZDataMax();
            float[] zlevels = new float[_mnumberContours];
            for (int i = 0; i < _mnumberContours; i++)
            {
                zlevels[i] = zmin + i * (zmax - zmin) / (_mnumberContours - 1);
            }
            ...
            for (int i = 0; i < pts.GetLength(0) - 1; i++)
            {
                for (int j = 0; j < pts.GetLength(1) - 1; j++)
                {
                    ...
                    for (int k = 0; k < _mnumberContours; k++)
                    {
                        // Left triangle:
                        i0 = i;
                        j0 = j;
        ...
        private void AddSlice(Graphics g, DataSeries ds, ChartStyle cs, ChartStyle2D cs2d)
        {
            ...
            for (int i = 0; i < pts.GetLength(0); i++)
            {
                for (int j = 0; j < pts.GetLength(1); j++)
                {
                    ...
                    {
                        pts[i, j, k]._mpoint3.Transform(m, form1, cs);
                        pts[i, j, k]._mpoint3.Z = pts[i, j, k]._mV;
                    }
                }
            }
            ...
            if (XYZSlice == SliceEnum.XSlice)
            {
                ...
                for (int j = 0; j < pts.GetLength(1)-1; j++)
                {
                    ...
                    {
                        pta[0] = new PointF(pts[nfix, j, k]._mpoint3.X, pts[nfix, j, k]._mpoint3.Y);
                        pta[1] = new PointF(pts[nfix, j+1, k]._mpoint3.X, pts[nfix, j+1, k]._mpoint3.Y);
                        pta[2] = new PointF(pts[nfix, j+1, k+1]._mpoint3.X, pts[nfix, j+1, k+1]._mpoint3.Y);
                        pta[3] = new PointF(pts[nfix, j, k+1]._mpoint3.X, pts[nfix, j, k+1]._mpoint3.Y);
                        Color color = AddColor(cs, pts[nfix, j, k]._mpoint3, vmin, vmax);
                        aBrush = new SolidBrush(color);
                        g.FillPolygon(aBrush, pta);
                        if (ds.LineStyle.IsVisible)
                        {
            ...
            else if (XYZSlice == SliceEnum.YSlice)
            {
                ...
                for (int i = 0; i < pts.GetLength(0) - 1; i++)
                {
                    for (int k = 0; k < pts.GetLength(2) - 1; k++)
                    {
                        pta[0] = new PointF(pts[i, nfix, k]._mpoint3.X, pts[i, nfix, k]._mpoint3.Y);
                        pta[1] = new PointF(pts[i + 1, nfix, k]._mpoint3.X, pts[i + 1, nfix, k]._mpoint3.Y);
                        pta[2] = new PointF(pts[i + 1, nfix, k + 1]._mpoint3.X, pts[i + 1, nfix, k + 1]._mpoint3.Y);
                        pta[3] = new PointF(pts[i, nfix, k + 1]._mpoint3.X, pts[i, nfix, k + 1]._mpoint3.Y);
                        Color color = AddColor(cs, pts[i,nfix, k]._mpoint3, vmin, vmax);
            ...
            else if (XYZSlice == SliceEnum.ZSlice)
            {
                ...
                for (int i = 0; i < pts.GetLength(0) - 1; i++)
                {
                    for (int j = 0; j < pts.GetLength(1) - 1; j++)
                    {
                        pta[0] = new PointF(pts[i, j,nfix]._mpoint3.X, pts[i,j, nfix]._mpoint3.Y);
                        pta[1] = new PointF(pts[i + 1, j,nfix]._mpoint3.X, pts[i + 1,j, nfix]._mpoint3.Y);
                        pta[2] = new PointF(pts[i + 1, j+1,nfix]._mpoint3.X, pts[i + 1,j+1, nfix]._mpoint3.Y);
                        pta[3] = new PointF(pts[i, j+1,nfix]._mpoint3.X, pts[i, j+1,nfix]._mpoint3.Y);
                        Color color = AddColor(cs, pts[i, j,nfix]._mpoint3, vmin, vmax);
                        aBrush = new SolidBrush(color);
                        g.FillPolygon(aBrush, pta);
                        if (ds.LineStyle.IsVisible)
                        {
