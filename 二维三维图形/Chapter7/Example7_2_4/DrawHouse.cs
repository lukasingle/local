...
namespace Example7_2_4
{
    public class DrawHouse
    {
        ...
        public void DrawCavalierView(Graphics g, float theta)
        {
            Point3[] pts = HouseCoordinates();
            Matrix3 m = Matrix3.Oblique(45, theta);
            for (int i = 0; i < pts.Length; i++)
            {
                pts[i].Transform(m);
            }
        ...
        public void DrawCabinetView(Graphics g, float theta)
        {
            Point3[] pts = HouseCoordinates();
            Matrix3 m = Matrix3.Oblique(63.4f, theta);
            for (int i = 0; i < pts.Length; i++)
            {
                pts[i].Transform(m);
            }
        ...
        public void DrawObliqueView(Graphics g, float alpha, float theta)
        {
            Point3[] pts = HouseCoordinates();
            Matrix3 m = Matrix3.Oblique(alpha, theta);
            for (int i = 0; i < pts.Length; i++)
            {
                pts[i].Transform(m);
            }
        ...
        public void DrawPerspectiveView(Graphics g, float d)
        {
            Point3[] pts = HouseCoordinates();
            // Translate the center of the house to the origin:
            Matrix3 m = Matrix3.Translate3(-pts[1].X / 2, 
                -pts[1].X / 2, pts[1].X / 2);
            for (int i = 0; i < pts.Length; i++)
            {
                pts[i].Transform(m);
            ...
            pta[1] = Point2D(new PointF(pts[4].X, pts[4].Y));
            pta[2] = Point2D(new PointF(pts[5].X, pts[5].Y));
            pta[3] = Point2D(new PointF(pts[9].X, pts[9].Y));
            g.FillPolygon(Brushes.LightGray, pta);
            g.DrawPolygon(Pens.Black, pta);
            pta = new PointF[4];
            pta[0] = Point2D(new PointF(pts[0].X, pts[0].Y));
            pta[1] = Point2D(new PointF(pts[1].X, pts[1].Y));
            pta[2] = Point2D(new PointF(pts[8].X, pts[8].Y));
            pta[3] = Point2D(new PointF(pts[9].X, pts[9].Y));
            g.FillPolygon(Brushes.LightGray, pta);
            g.DrawPolygon(Pens.Black, pta);
            pta = new PointF[4];
            ...
            pta[4] = Point2D(new PointF(pts[4].X, pts[4].Y));
            g.FillPolygon(Brushes.LightGray, pta);
            g.DrawPolygon(Pens.Black, pta);
        }
